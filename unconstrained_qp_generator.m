...
%% set parameters here
    ...
    % [x0;xt;vx0;vxt;ax0;axt;w0;wt];
%     segments0 = [0 4 0 0 0 0 0 0 
%                 4 11 0 0 0 0 0 0 
%                 11 17 0 0 0 0 0 0
%                 ];
...
%%  matrix initial
    ...
    poly1 = poly0;
    segments1 = zeros(1,num_st*2*dim*num_seg);
    path1 = zeros(frame*num_seg+1,2);
...
%     end
    ...
    for is = 1:num_seg
        for index=1+(is-1)*frame:frame+(is-1)*frame
            ...
            for i=0:N   % x path
                path1(index,1) = t;
                path1(index,2) = poly1(i+1+(is-1)* (N+1))*t^(i) + path1(index,2);  %x
            end
        end
    end
    path1(frame*num_seg+1,:) = [t+T/frame segments1(1,end-num_st+1)];
